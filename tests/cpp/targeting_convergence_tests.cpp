...
void testStreamSharpnessConvergesWithoutSpeedCapDependency()
{
    ...
    const StepConvergenceMetrics tuned = simulateStreamStep(120.0, 36.0, 1800.0);

    REQUIRE(legacy.timeToTwoPxMs > 210.0);
    REQUIRE(tuned.timeToTwoPxMs > 0.0);
    REQUIRE(tuned.timeToTwoPxMs <= 130.0);
    REQUIRE(tuned.timeToOnePxMs <= 160.0);
    REQUIRE(tuned.timeToTwoPxMs < legacy.timeToTwoPxMs * 0.60);
