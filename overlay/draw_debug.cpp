...

    {
        ...
        for (size_t i = 0; i < detectionBuffer.boxes.size(); ++i)
        {
            ...
            if (i < detectionBuffer.classes.size())
            {
                ...
                {
                    label += " ";
                    label += std::to_string(static_cast<int>(std::round(detectionBuffer.confidences[i] * 100.0f)));
                    label += "%";
                }
...
void draw_debug()
{
    ...
    if (OverlayUI::BeginSection("Log File", "debug_section_log_file"))
    {
        if (ImGui::Checkbox("Enable log file", &config.debug_log_file_enabled))
        {
            appLogger.configure(config.debug_log_file_enabled, config.debug_log_file_path);
            appLogger.log("[Debug] File logging enabled.");
            OverlayConfig_MarkDirty();
        }
        ImGui::SetNextItemWidth(OverlayUI::AdaptiveItemWidth(0.78f));
        if (ImGui::InputText("Log file path", debugLogFilePathBuf, IM_ARRAYSIZE(debugLogFilePathBuf)))
        {
            config.debug_log_file_path = debugLogFilePathBuf;
            appLogger.configure(config.debug_log_file_enabled, config.debug_log_file_path);
            appLogger.log("[Debug] Log file path updated.");
            OverlayConfig_MarkDirty();
        }
        OverlayUI::EndSection();
    }
