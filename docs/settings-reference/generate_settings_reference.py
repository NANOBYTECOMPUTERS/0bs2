...
GUI_CONTROLS: list[dict[str, str]] = [
    ...
    row("Mouse", "Pure PID Movement", "Feed-forward gain", "pid_feed_forward_gain", "Slider", "0.000-2.000", "Strength of feed-forward movement.", "overlay/draw_mouse.cpp"),
...
OTHER_GUI: list[dict[str, str]] = [
    ...
    row("Mouse", "Input Method", "Mouse Input Method", "input_method", "Combo", "WIN32, GHUB, RAZER, ARDUINO, KMBOX_NET, KMBOX_A, MAKCU", "Changes active mouse backend.", "overlay/draw_mouse.cpp"),
    row("Mouse", "Input Method", "Arduino Port", "arduino_port", "Combo", "COM1-COM30", "Arduino input only.", "overlay/draw_mouse.cpp"),
    row("Mouse", "Input Method", "Arduino Baudrate", "arduino_baudrate", "Combo", "9600-115200", "Arduino input only.", "overlay/draw_mouse.cpp"),
...
CONFIG_ONLY: list[dict[str, str]] = [
    ...
    config_only("Kalman", "kalman_enabled", "true", "true/false", "Enables Kalman filtering for target motion estimation.", "Loadable hidden key; no GUI control."),
    config_only("Kalman", "kalman_process_noise_position", "40.0", "0.0001-5000", "Position process noise. Higher values make the filter adapt faster to position changes.", "Loadable hidden key; no GUI control."),
    config_only("Kalman", "kalman_process_noise_velocity", "1800.0", "0.0001-50000", "Velocity process noise. Higher values make velocity estimates change faster.", "Loadable hidden key; no GUI control."),
    config_only("Kalman", "kalman_measurement_noise", "35.0", "0.0001-5000", "Measurement noise. Higher values trust detections less.", "Loadable hidden key; no GUI control."),
    config_only("Kalman", "kalman_velocity_damping", "0.08", "0.0-3.0", "Damps velocity estimates to reduce runaway prediction.", "Loadable hidden key; no GUI control."),
    config_only("Kalman", "kalman_max_velocity", "20000.0", "100-60000", "Maximum allowed tracked velocity.", "Loadable hidden key; no GUI control."),
    config_only("Kalman", "kalman_warmup_frames", "2", "0-20", "Frames before Kalman output is considered warmed up.", "Loadable hidden key; no GUI control."),
    config_only("Kalman", "kalman_compensate_detection_delay", "true", "true/false", "Accounts for detector latency in prediction.", "Loadable hidden key; no GUI control."),
    config_only("Kalman", "kalman_additional_prediction_ms", "0.0", "-80.0-120.0", "Manual extra prediction offset in milliseconds.", "Loadable hidden key; no GUI control."),
    config_only("Kalman", "kalman_reset_timeout_sec", "0.5", "0.05-3.0", "Time without stable observations before Kalman state resets.", "Loadable hidden key; no GUI control."),
